...
<style id="child-drawer-style">
  ...
  /* archify's stock .container caps at 1440px (a "reading width" meant for
     prose-shaped diagrams) regardless of how wide/tall the actual content
     is. A dense diagram (many lanes, one node per real function call) is
     not prose — capping it at 1440px on a wide monitor just adds two huge
     empty gutters and shrinks every node's text for no reason. Only widen
     it, never narrow below the stock default. */
  .container { max-width: max(1440px, min(94vw, 2200px)) !important; }

  /* The line above fixed the gutters but created a NEW mismatch: archify's SVG
     fills 100% of .container width against a fixed authored viewBox, so widening
     .container scales every diagram font up with it (verified via
     getBoundingClientRect: ~1.96x at a 1805px container here, vs ~1.59x at the
     stock 1440px) -- but the page's own title/subtitle/toolbar are ordinary HTML
     text at fixed px, so they stay the same size while the diagram keeps growing
     on wider screens. The mismatch gets worse the wider the screen, not better.
     Scale them with vw (like the SVG effectively does) instead of fixed px, so
     the header/toolbar and the diagram grow together and stay proportioned to
     each other at any viewport width the .container override above can reach.
     The clamp() floor matches archify's original fixed sizes so nothing shrinks
     below stock on a narrow screen. */
  .header h1 { font-size: clamp(24px, 1.6vw, 34px) !important; }
  .header .subtitle { font-size: clamp(13px, 0.85vw, 17px) !important; }
  .toolbar button { font-size: clamp(12px, 0.8vw, 16px) !important; }
  /* archify's stock PATH/MAP/LENS + zoom nav (bottom-right, floating over the
     diagram) ships at a fixed 0.56rem (~9px) regardless of container width --
     it was always small by archify's own design, but it reads as flat-out
     broken now that everything around it scales up with the widened
     container. Bring it into the same vw-scaled family, just pegged smaller
     since these are meant to stay compact icon-sized controls. */
  .diagram-nav button { font-size: clamp(11px, 0.7vw, 14px) !important; }
