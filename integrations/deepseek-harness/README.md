...
v0.2.0 targets experimental compatibility with developer-preview **`@deepseek-ai/dsh@0.1.2-rc.1`** on Node.js **`^22.19.0 || >=24.0.0`**. It is not a stable cross-version guarantee.

The package is a Skill-only bundle: it inserts one filesystem Skill provider named `archify-plugin` and exposes an **Archify 2.17.0-dev.1 development snapshot**, pinned to commit `920543baa1c6137803c5b45a69d8977152773d35`. This is not an Archify 2.17 stable release. Compared with plugin 0.1.0 (Archify 2.14.0), it includes authored brand marks, Workflow schema v2, Viewer localization, update awareness, and the subsequent packaging and CLI receipt fixes.
...
```bash
dsh plugin --profile web add @tt-a1i/archify-dsh@0.2.0
```
...
For a new adapter release, bump `package.json`, update `release.json` with the full source commit and matching Skill/DSH versions, and update the installation examples. Run:
